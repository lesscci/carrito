<div>
    ...
    <div class="px-6 py-4 flex items-center">
        <x-secondary-button wire:click="$set('open_create', true)" class="px-6 py-4">
            Crear producto
        </x-secondary-button>
    </div>
        ...
        @foreach ($cartItems as $item)
            <li class="flex justify-between gap-x-6 py-5">
                <div class="flex gap-x-4">
                    <img src="{{ asset('storage/' . basename($item->imagen)) }}" style="width: 90px; height:100px;">
                    <div class="min-w-0 flex-auto">
                        ...
                        <p class="mt-1 truncate text-sm leading-5 text-gray-500">Categoria:
                            {{ $item->categoria->nombre }}</p>
                    </div>
                </div>
                <div class="hidden sm:flex sm:flex-col sm:items-end">
                    <a href="#" wire:click="edit({{ $item->id }})"
                        class="text-sm leading-6 text-gray-900">Editar</a>
                    <a href="#" wire:click="delete({{ $item->id }})"
                        class="text-sm leading-6 text-gray-900">Eliminar</a>
                </div>
            </li>
        @endforeach
    </ul>
    ...
    <x-dialog-modal wire:model="open_create">
        ...
        <x-slot name='footer'>
            ...
            <x-danger-button wire:click="$set('open_create', false)">
                Cancelar
            </x-danger-button>
        </x-slot>
    </x-dialog-modal>
